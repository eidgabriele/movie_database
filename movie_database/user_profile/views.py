...
def register(request):
    if request.method == "POST":
        ...
        if not email or User.objects.filter(email=email).first():
            messages.error(request, 'Invalid email, user with this email already exists.')
            error = True
        else:
            try:
        ...
        if not password or not password2 or password != password2:
            messages.error(request, 'Passwords not entere, or do not match')
            error = True
        if not error:
            User.objects.create_user(username=username, email=email, password=password)
            messages.success(request, f'User {username} registration was successful. You can login now')
